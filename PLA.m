...
k = 1;
count = 1;
plPar = nan.*zeros(nr,1);
plRes = nan.*zeros(nr,1);
plPar(1) = par(i);
plRes(1) = res;
max_adjust = false; %prevents infinite while loop

while ((plPar(k)<=maxPar && logical(flag+1) && count<=nr) || (plPar(k)>=maxPar && ~logical(flag+1) && count<=nr))
    % take a step along the parameter 
    tempPar = flag*step+plPar(k);
    
    [x,resnorm] = lsqnonlin( @(nPar)optFun(nPar,tempPar),nPar(k,:),lb,ub,Optimoptions);
    
    % change of the resnorm value
    resChange = abs(resnorm - plRes(k));
...
%     end
    ...
    if step == minStep & resChange > maxChange
        tempChange = maxChange;
        maxChange = resChange;
        max_adjust = true;
    end
